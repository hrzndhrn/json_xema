defmodule Draft4.Optional.FormatTest do
  ...
  describe "validation of date-time strings" do
    setup do
      %{schema: JsonXema.new(~s( {"format":"date-time"} ))}
    end
  ...
  describe "validation of URIs" do
    setup do
      %{schema: JsonXema.new(~s( {"format":"uri"} ))}
    end
  ...
  describe "validation of e-mail addresses" do
    setup do
      %{schema: JsonXema.new(~s( {"format":"email"} ))}
    end
  ...
  describe "validation of IP addresses" do
    setup do
      %{schema: JsonXema.new(~s( {"format":"ipv4"} ))}
    end
  ...
  describe "validation of IPv6 addresses" do
    setup do
      %{schema: JsonXema.new(~s( {"format":"ipv6"} ))}
    end
  ...
  describe "validation of host names" do
    setup do
      %{schema: JsonXema.new(~s( {"format":"hostname"} ))}
    end
